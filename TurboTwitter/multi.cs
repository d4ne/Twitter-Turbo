...
namespace TurboTwitter
{
    public partial class multi : UserControl
    {
        ...
        private void bunifuFlatButton2_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please upload a Userlist containing atleast 1 Username.", "Twitter Turbo Error");
            }
            else
            {
        ...
        private void multi_Load(object sender, EventArgs e)
        {
            ...
            dataGridView1.Rows[0].Cells[0].Value = "Please add a userlist";
        }
            ...
            Parallel.ForEach(dataGridView1.Rows.Cast<DataGridViewRow>(), (row, loopState) =>
            {
                ...
                else
                {
                    try
                    {
                        ...
                        string[] textArray1 = new string[] { "_method=PUT&authenticity_token=", str2, "&orig_uname=", row.Cells[0].Value.ToString(), "&orig_email=", str4, "&user%5Bscreen_name%5D=", row.Cells[0].Value.ToString(), "&user%5Bemail%5D=", str4, "&user%5Blang%5D=en&user%5Btime_zone%5D=Pacific+Time+%28US+%26+Canada%29&user%5Bcountry%5D=us&user%5Bnsfw_view%5D=0&user%5Bnsfw_user%5D=0&user%5Bautoplay_disabled%5D=0&user%5Bautoplay_disabled%5D=1&auth_password=", password };
                        ...
                        while (!userChanged)
                        {
                            try
                            {
                                ...
                                if (client1.DownloadString("https://twitter.com/" + row.Cells[0].Value.ToString()).Contains("Sorry, that page doesn’t exist!"))
                                {
                                    if (!userChanged)
                                    {
                                        ...
                                        HttpWebResponse response2 = (HttpWebResponse)httpWebRequest_0.GetResponse();
                                        ...
                                        client2.Headers["Accept-Language"] = "en-us";

                                        if (finalStr.Contains("user-style-" + row.Cells[0].Value.ToString()) && username != row.Cells[0].Value.ToString())
                                        {
                                            this.Invoke((MethodInvoker)delegate
                                            {
                                                UpdateTurboStatus(false, row.Cells[0].Value.ToString());
                                            });
        ...
        bool UpdateTurboTrys(bool status, string handle)
        {
            if (status == true)
            {
                var x = Convert.ToInt32(bunifuCustomLabel10.Text);
                var y = x + 1;

                bunifuCustomLabel10.Text = y.ToString();
                bunifuCustomLabel10.Refresh();
                return true;
            }
            else
            {
        ...
        private void doLogin()
        {
            ...
            try
            {
                ...
                string[] textArray1 = new string[] { "session%5Busername_or_email%5D=", username, "&session%5Bpassword%5D=", password, "&authenticity_token=", token, "&scribe_log=&redirect_after_login=%2F&authenticity_token=", token };
                byte[] bytes = Encoding.ASCII.GetBytes(string.Concat(textArray1));
